    ...
    for flavor, level in flavordict.items():
        ...
        for ingredient, amount in ingredients.items():
            if ingredient in response:
                ...
                response[ingredient] = amount
    return response
# Let's simulate the new input with noodle style "น้ำตก" and the provided flavor profile.

input_data_2 = {
    "noodle_style": "น้ำใส",
    "noodle_type": "หมี่เหลือง",
    "cup_size": 2,  # "ใหญ่" cup size
    "flavors": {
        "Sweetness Level": "75",   # 75% sweetness
        "Sourness Level": "25",    # 25% sourness
        "Saltiness Level": "50",   # 50% saltiness
        "Spiciness Level": "0"     # 0% spiciness
    }
}

# Running the display_recipe function to generate the recipe output
response_2 = display_recipe(input_data_2["flavors"], input_data_2["cup_size"], input_data_2["noodle_type"], soup_type="น้ำตก")
